...
def test_working_signal_uses_soft_green_pulse() -> None:
    ...
    assert len(light.brightness_states) == 9
    assert all(green > 0 and yellow == 0 and red == 0 for green, yellow, red in light.brightness_states)
...
def test_is_running_posix_handles_oserror(monkeypatch) -> None:
    monkeypatch.setattr(runtime.os, "name", "posix")

    def fake_kill(_pid: int, _sig: int) -> None:
        raise OSError("unsupported signal")

    monkeypatch.setattr(runtime.os, "kill", fake_kill)

    assert runtime._is_running(12345) is False


def test_is_running_windows_uses_tasklist(monkeypatch) -> None:
    ...
    assert runtime._is_running(12345) is True
    assert calls == [["tasklist", "/FI", "PID eq 12345", "/FO", "CSV", "/NH"]]


def test_is_running_windows_returns_false_for_missing_pid(monkeypatch) -> None:
    monkeypatch.setattr(runtime.os, "name", "nt")
...
def test_terminate_windows_uses_taskkill(monkeypatch) -> None:
    monkeypatch.setattr(runtime.os, "name", "nt")
    ...
    monkeypatch.setattr(runtime, "_is_windows_process_running", lambda _pid: next(running))
    monkeypatch.setattr(runtime.subprocess, "run", fake_run)
