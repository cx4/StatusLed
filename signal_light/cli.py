...
def _add_transport_options(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--backend",
        choices=("http", "serial", "ble"),
        help="hardware backend; defaults to env auto-detection or http",
    )
    parser.add_argument("--http-url", help="ESP32-C3 base URL, for example http://192.168.4.1")
    parser.add_argument("--serial-port", help="ESP32-C3 serial port, for example COM5 or /dev/ttyACM0")
    parser.add_argument("--serial-baud", type=int, help="ESP32-C3 serial baud rate")
    parser.add_argument("--ble-name", help="ESP32-C3 BLE device name; defaults to rgb-c3")
    parser.add_argument("--ble-address", help="ESP32-C3 BLE address; skips name scanning when provided")


def _apply_transport_options(args: argparse.Namespace) -> None:
    updates = {
        "SIGNAL_LIGHT_BACKEND": getattr(args, "backend", None),
        "SIGNAL_LIGHT_HTTP_URL": getattr(args, "http_url", None),
        "SIGNAL_LIGHT_SERIAL_PORT": getattr(args, "serial_port", None),
        "SIGNAL_LIGHT_SERIAL_BAUD": getattr(args, "serial_baud", None),
        "SIGNAL_LIGHT_BLE_NAME": getattr(args, "ble_name", None),
        "SIGNAL_LIGHT_BLE_ADDRESS": getattr(args, "ble_address", None),
    }
