...
def _default_state_dir() -> str:
    if os.name == "nt":
        return str(Path(tempfile.gettempdir()) / "signal-light")
    return "/private/tmp/signal-light" if sys.platform == "darwin" else "/tmp/signal-light"


STATE_DIR = Path(os.environ.get("SIGNAL_LIGHT_STATE_DIR", _default_state_dir()))
...
def _terminate_once(pid: int) -> None:
    if os.name == "nt":
...
def _terminate_windows_process(pid: int) -> None:
    ...
    raise PermissionError(detail or f"taskkill failed with exit code {result.returncode}")
